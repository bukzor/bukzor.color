...
@example(
    hue=Decimal("120"), luminance=Decimal("1.0")
)  # Should be white regardless of hue
@example(
    hue=Decimal("240"), luminance=Decimal("0.5")
)  # Should be gray regardless of hue
def test_wcag_hcl_achromatic_preserves_luminance(
    hue: Decimal, luminance: Decimal
):
    ...
    color = WcagHCLEncoding(
        Degrees(hue), Percentage(Decimal("0")), Luminance(luminance)
    ).decode()

    # Should preserve the exact luminance
    actual_luminance = color.luminance()
    assert abs(actual_luminance - luminance) < Decimal(
        "0.001"
    ), f"h={hue}, c=0, l={luminance} -> actual luminance {actual_luminance}"

    # Should be achromatic (gray/white/black)
    r, g, b = color.red, color.green, color.blue
    assert abs(r - g) < Decimal(
        "0.001"
    ), f"Should be achromatic: R={r}, G={g}, B={b}"
    assert abs(g - b) < Decimal(
        "0.001"
    ), f"Should be achromatic: R={r}, G={g}, B={b}"
...
def test_wcag_hcl_round_trip(r: Decimal, g: Decimal, b: Decimal):
    ...
    assert abs(reconstructed_color.red - r) < Decimal(
        "0.001"
    ), f"Red: {r} -> {reconstructed_color.red}"
    assert abs(reconstructed_color.green - g) < Decimal(
        "0.001"
    ), f"Green: {g} -> {reconstructed_color.green}"
    assert abs(reconstructed_color.blue - b) < Decimal(
        "0.001"
